...
def rnn(input_data, weights, biases):
    ...
    # basic LSTM Cell.
    lstm_cell = tf.contrib.nn.rnn_cell.BasicLSTMCell(n_hidden_units, forget_bias=1.0, state_is_tuple=True)
    ...
    outputs = tf.contrib.nn.unpack(tf.transpose(outputs, [1, 0, 2]))    # states is the last outputs
